...
import { MapPin, Truck, Tag, ClipboardList, Cpu, ShoppingBag, LogOut } from 'lucide-react'
import { useAuthStore } from '@/stores/auth'

const navItems = [
  { to: '/destinations',  icon: MapPin,         label: '地點管理' },
  { to: '/vehicles',      icon: Truck,          label: '車輛管理' },
  { to: '/vehicle-types', icon: Tag,            label: '車輛類型' },
  { to: '/bento-orders',  icon: ShoppingBag,    label: '便當訂單' },
  ...
  return (
    <aside className="flex flex-col w-64 min-h-full bg-base-100 border-r border-base-300">
      ...
      <nav className="flex-1 px-3 py-3">
        <p className="text-[10px] font-semibold text-base-content/35 uppercase tracking-widest px-3 mb-2">管理</p>
