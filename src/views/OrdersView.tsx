    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-base-content/50 text-sm mt-1">建立與管理 VRP 計算任務組</p>
        </div>
              ...
              <tbody>
                {orders.length === 0 ? (
                  ...
                  <tr key={order.id} className="hover:bg-base-50 border-b border-base-100 last:border-0">
                    <td className="font-medium font-mono text-sm">{order.id.slice(0, 8)}...</td>
                    ...
                    <td>
                      <div className="flex justify-end gap-1">
                        <button className="btn btn-ghost btn-xs tooltip" data-tip="查看詳情" onClick={() => openDetail(order)}>
                          <Eye className="w-3.5 h-3.5" />
                        </button>
                        <button className="btn btn-ghost btn-xs tooltip" data-tip="前往計算" onClick={() => goToComputes(order.id)}>
      ...
      <OrderFormModal open={showFormModal} onClose={() => setShowFormModal(false)} />

      <OrderDetailModal open={showDetailModal} order={selectedOrder} onClose={() => setShowDetailModal(false)} />
