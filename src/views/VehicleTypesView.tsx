...
export default function VehicleTypesView() {
  ...
  function openCreate() { setEditingType(null); setShowModal(true) }
  function openEdit(t: VehicleType) { setEditingType(t); setShowModal(true) }
    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-base-content/50 text-sm mt-1">定義車輛類別，供車輛設定中使用</p>
        </div>
      ...
      {!isLoading && !error && types.length === 0 && (
        <div className="card bg-base-100 shadow-sm border border-base-200">
          <div className="card-body items-center text-center py-16">
            ...
            <p className="text-sm text-base-content/40 max-w-xs mt-1">建立車輛類型後，可在車輛設定中指定車輛的類別</p>
            <button className="btn btn-primary btn-sm mt-4 gap-2" onClick={openCreate}>
              <Plus className="w-3.5 h-3.5" />建立第一個類型
            </button>
          </div>
        </div>
      )}
      ...
      {!isLoading && !error && types.length > 0 && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {types.map((t) => (
              <div key={t.id} className="card bg-base-100 shadow-sm border border-base-200 hover:shadow-md hover:border-base-300 transition-all">
                <div className="card-body p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-0.5 shrink-0">
                      <button className="btn btn-ghost btn-xs tooltip" data-tip="編輯" onClick={() => openEdit(t)}>
                        <Pencil className="w-3 h-3" />
                      </button>
                      <button className="btn btn-ghost btn-xs text-error/50 hover:text-error tooltip" data-tip="刪除" onClick={() => confirmDelete(t)}>
          ...
          <p className="mt-3 text-xs text-base-content/40 px-1">共 {types.length} 種車輛類型</p>
        </>
      )}

      <VehicleTypeModal open={showModal} editData={editingType} onClose={() => setShowModal(false)} />

      <dialog ref={deleteDialogRef} className="modal" onClose={() => setDeletingType(null)}>
        <div className="modal-box max-w-sm">
          <h3 className="font-semibold text-base mb-2">確認刪除</h3>
          <p className="text-sm text-base-content/70">
            確定要刪除車輛類型「<strong>{deletingType?.name}</strong>」嗎？此操作無法復原。
          </p>
          <div className="modal-action mt-5 pt-4 border-t border-base-200">
            ...
            <button className="btn btn-error btn-sm" disabled={deleteType.isPending} onClick={handleDelete}>
